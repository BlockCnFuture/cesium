...
BillboardCollection.prototype.update = function (frameState) {
  ...
  function createShaderProgram(entity, context, attributeLocations) {
    ...
    if (isVectorTile) {
      ...
      fragmentSource = entity._batchTable.getFragmentShaderCallback(
        false,
        undefined,
      )(fragmentSource);
    }
    ...
    if (entity._shaderClampToGround) {
      vertexShader.defines.push("VS_THREE_POINT_DEPTH_CHECK");
    }
    ...
    if (entity._blendOption === BlendOption.OPAQUE_AND_TRANSLUCENT) {
      opaqueFragmentShader = new ShaderSource({
        defines: ["TRANSLUCENT", "DEPTH_FAIL_TRANSLUCENCY"],
        sources: [fragmentSource],
      });

      if (entity._shaderClampToGround) {
        opaqueFragmentShader.defines.push("VS_THREE_POINT_DEPTH_CHECK");
      }
      if (entity._sdf) {
      ...
      if (entity._shaderClampToGround) {
        translucentFragmentShader.defines.push("VS_THREE_POINT_DEPTH_CHECK");
      }
      if (entity._sdf) {
    ...
    if (entity._blendOption === BlendOption.OPAQUE) {
      opaqueFragmentShader = new ShaderSource({
        defines: ["DEPTH_FAIL_TRANSLUCENCY"],
        sources: [fragmentSource],
      });
